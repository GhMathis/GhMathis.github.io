...
Création de la fonction qui calcule chaque pour chaque valeurs de traits de x et y la fitness des deux phénotype. Un argument sigma est ajouté pour observé la variation de fitness pour différentes paramétres de la fonction d'inteesité de compétition.
...
Pour 50 phénotype en compétition et avec $\sigma = 0.2$. On devrais obtenir un équilibre
...
On ajoute maintenant la mutation. Pour cela on va considéré un temps évolutif, différent du temps de la dynamique. Ce temp évolutif est discret, une unité de temp évolutif correspond a une dynamique complété. On commence la simulation avec un seul phénotype. A chaque pas de temps évolutif une partie de la population du phénotype peut muté. Cella ce traduit par un transfert d'une la population vers une valeur de trait légèrement plus ou légèrement moins élever.A la fin de chaque dynamique on récupéré les valeurs abondance de l'ensemble des trait présent, ces valeur sont le récapitulatif de chaque pas de temp évolutif et sont aussi les valeurs initial de la dynamique du prochain temps évolutif.

Fonction de dynamique adatpative avec mutation
...
mutation.dyna.adapt = function(ini = 1, sigma){  
  ...
  T_evolutif = 250 #nbr de pas de temps évolutifs
...
# Initialisation des index max et min pour les valeurs traits ( utilisé pour ne pas arrêt la boucle lorsque un trait mute en dehors des valeurs de traits possibles )
  ...
  for(T_evo in 1:(T_evolutif-1)){
    ...
    dyna_glv = dyna_glv[,2:(N+1)] #retrait de la colonne t
...
On lance les simulations pour différentes valeur de sigma, avec seulement 1 phénotype a l'état initial
...
ggplot(dyna_mutation_long)+
  ...
  facet_wrap(~sigma, labeller = labeller(sigma = c("sig",  label_value)))+
  scale_fill_gradient2(low = "white" ,
  high = "black")+
